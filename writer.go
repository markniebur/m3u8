...
// LastIndex returns the previously written segment's index
func (p *MediaPlaylist) LastIndex() uint {
	if p.tail == 0 {
		return p.capacity - 1
	}
	return p.tail - 1
...
func (p *MediaPlaylist) SetKey(method, uri, iv, keyformat, keyformatversions string) error {
	...
	p.Segments[p.LastIndex()].Key = &Key{method, uri, iv, keyformat, keyformatversions}
	return nil
}
...
func (p *MediaPlaylist) SetMap(uri string, limit, offset int64) error {
	...
	version(&p.ver, 5) // due section 4
	p.Segments[p.LastIndex()].Map = &Map{uri, limit, offset}
	return nil
}
...
func (p *MediaPlaylist) SetRange(limit, offset int64) error {
	...
	p.Segments[p.LastIndex()].Limit = limit
	p.Segments[p.LastIndex()].Offset = offset
	return nil
}
...
func (p *MediaPlaylist) SetSCTE35(scte35 *SCTE) error {
	...
	p.Segments[p.LastIndex()].SCTE = scte35
	return nil
}
...
func (p *MediaPlaylist) SetDiscontinuity() error {
	...
	p.Segments[p.LastIndex()].Discontinuity = true
	return nil
}
...
func (p *MediaPlaylist) SetProgramDateTime(value time.Time) error {
	...
	p.Segments[p.LastIndex()].ProgramDateTime = value
	return nil
}
