...
func decodeLineOfMediaPlaylist(p *MediaPlaylist, wv *WV, state *decodingState, line string, strict bool) error {
	...
	case !strings.HasPrefix(line, "#"):
		...
		if state.tagKey {
			p.Segments[p.LastIndex()].Key = &Key{state.xkey.Method, state.xkey.URI, state.xkey.IV, state.xkey.Keyformat, state.xkey.Keyformatversions}
		...
		if state.tagMap {
			p.Segments[p.LastIndex()].Map = &Map{state.xmap.URI, state.xmap.Limit, state.xmap.Offset}
		...
		if state.tagCustom {
			p.Segments[p.LastIndex()].Custom = state.custom
			state.custom = make(map[string]CustomTag)
